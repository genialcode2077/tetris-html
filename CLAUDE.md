...
- Para ver el resultado: `pnpm dev` en segundo plano y abrir `http://localhost:5173` con las herramientas de Chrome (`tabs_create_mcp` → `navigate` → `screenshot`); revisar consola con `read_console_messages` (pattern `error|warn`).
